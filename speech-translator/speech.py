...
def get_wave_header(frame_rate):
    ...
    #output = BytesIO()
    output.write('RIFF')
    output.write(struct.pack('<L', 0).__str__())
    output.write('WAVE')
    output.write('fmt ')
    output.write(struct.pack('<L', 18).__str__())
    output.write(struct.pack('<H', 0x0001).__str__())
    output.write(struct.pack('<H', nchannels).__str__())
    output.write(struct.pack('<L', frame_rate).__str__())
    output.write(struct.pack('<L', frame_rate * nchannels * bytes_per_sample).__str__())
    output.write(struct.pack('<H', nchannels * bytes_per_sample).__str__())
    output.write(struct.pack('<H', bytes_per_sample * 8).__str__())
    output.write(struct.pack('<H', 0).__str__())
    output.write('data')
    output.write(struct.pack('<L', 0).__str__())
...
if __name__ == "__main__":
    ...
    request_url = "wss://dev.microsofttranslator.com/speech/translate?from={0}&to={1}&features={2}&api-version=1.0&voice={3}".format(translate_from, translate_to, features, voice)
